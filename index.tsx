...
document.getElementById('showProfileBtn')?.addEventListener('click', openProfileModal);
// Global variables
...
function showMultiSelectionPopup(): Promise<Selections | null> {
  return new Promise((resolve) => {
    ...
    popup.style.background = isLightMode ? 'var(--color-surface-light)' : 'var(--color-surface-dark)';
    popup.style.color = isLightMode ? 'var(--color-text-light)' : 'var(--color-text-dark)';
    popup.style.padding = '20px';
    popup.style.borderRadius = '12px';
    popup.style.boxShadow = isLightMode ? 'var(--shadow-md)' : 'var(--shadow-lg)';
    popup.style.border = `1px solid ${isLightMode ? 'var(--color-border-light)' : 'var(--color-border-dark)'}`;
    ...
    function createSelect(labelText: string, options: string[]): HTMLSelectElement {
      ...
      select.style.border = `1px solid ${isLightMode ? 'var(--color-border-light)' : 'var(--color-border-dark)'}`;
      select.style.background = isLightMode ? 'var(--color-bg-light)' : 'var(--color-bg-dark)';
      select.style.color = isLightMode ? 'var(--color-text-light)' : 'var(--color-text-dark)';
    ...
    okBtn.style.backgroundColor = isLightMode ? 'var(--color-accent-light)' : 'var(--color-accent-dark)';
    okBtn.style.color = '#fff';
    okBtn.style.cursor = 'pointer';

    // Activation du bouton valider uniquement si les 3 selections sont faites
    function checkSelections() {
      okBtn.disabled = !(goalSelect.value && industrySelect.value && roleSelect.value);
    }
...
// Fonction principale pour traiter l'enregistrement
async function processRecording(audioBlob: Blob): Promise<void> {
  ...
  // Affiche la popup et récupère les 3 choix
  ...
  try {
    ...
    if (!updatedSession) throw new Error('Échec de la mise à jour de la session');
...
export async function generateTitle(transcription: string, selections: Selections): Promise<string> {
  const prompt = `
...
export async function generateSummary(transcription: string, selections: Selections): Promise<string> {
  const prompt = `
Contexte professionnel : Secteur - ${selections.industry}, Rôle - ${selections.role}
Objectif : ${selections.goal}
...
export async function generateDetailedNote(transcription: string, selections: Selections): Promise<string> {
  const prompt = `
Contexte professionnel : Secteur - ${selections.industry}, Rôle - ${selections.role}
Objectif : ${selections.goal}
...
${transcription}
  ...
  const response = await openai.chat.completions.create({
    model: getModel(),
    messages: [
      { role: 'system', content: 'Tu es un assistant de rédaction professionnelle.' },
      { role: 'user', content: prompt }
    ]
  });
...
function showMessage(message, type = 'info') {
  ...
  popup.style.backgroundColor = type === 'error' ? '#f44336' : (type === 'success' ? '#4CAF50' : '#2196f3');
...
document.getElementById('previewPdfButton')?.addEventListener('click', async () => {
  ...
  try {
    ...
    showMessage('Erreur lors de la prévisualisation du fichier PDF.', 'error');
  }
});
...
async function translateWithLibreTranslate(text, targetLanguage, sourceLanguage = 'fr') {
  ...
  try {
    ...
    const texts = {
      fr: {
        ...
        greeting: 'Bonjour,',
        intro: 'Veuillez trouver ci-dessous le résumé généré automatiquement suite à votre dictée :',
        thanks: 'Nous vous remercions d’avoir utilisé <strong>DictateAI</strong>.',
      ...
      en: {
        ...
        greeting: 'Hello,',
        intro: 'Please find below the automatically generated summary of your dictation:',
      ...
      de: {
        ...
        greeting: 'Hallo,',
        intro: 'Nachfolgend finden Sie die automatisch generierte Zusammenfassung Ihres Diktats:',
      ...
      es: {
        ...
        greeting: 'Hola,',
        intro: 'A continuación encontrará el resumen generado automáticamente de su dictado:',
      ...
      it: {
        ...
        greeting: 'Ciao,',
        intro: 'Di seguito trovi il riepilogo generato automaticamente del tuo dettato:',
    ...
    showMessage('Résumé envoyé avec succès aux contacts sélectionnés et à vous.', 'success');

  } catch (error) {
    console.error('Erreur d\'envoi :', error);
    showMessage(error.message || 'Erreur inconnue lors de l’envoi des emails.', 'error');
  }
}
...
async function openContactSelectionPopup(sessionId) {
  ...
  modal.style.background = isLight ? 'var(--color-surface-light)' : 'var(--color-surface-dark)';
  modal.style.color = isLight ? 'var(--color-text-light)' : 'var(--color-text-dark)';
  modal.style.padding = '24px';
  modal.style.borderRadius = '12px';
  modal.style.boxShadow = isLight ? 'var(--shadow-md)' : 'var(--shadow-lg)';
  modal.style.border = `1px solid ${isLight ? 'var(--color-border-light)' : 'var(--color-border-dark)'}`;
  ...
  modal.innerHTML = `
    ...
    <select id="languageSelect" style="
      ...
      border: 1px solid ${isLight ? 'var(--color-border-light)' : 'var(--color-border-dark)'};
      border-radius: 6px;
    ">
    ...
    <div style="text-align: right;">
      ...
      <button id="confirmSendSummaryBtn" style="
        ...
        border: none;
        border-radius: 6px;
        background-color: ${isLight ? 'var(--color-accent-light)' : 'var(--color-accent-dark)'};
        color: white;
        cursor: pointer;
      ">Envoyer</button>
    </div>
    ...
    contactsListEl.innerHTML = filtered.map(c => 
      `<li style="margin-bottom: 8px;">
        <label style="cursor: pointer;">
          ...
          <strong>${c.prenom} ${c.nom}</strong> <span style="color: ${isLight ? '#666' : '#aaa'};">(${c.email})</span>
        </label>
      </li>`
    ).join('');
  }
